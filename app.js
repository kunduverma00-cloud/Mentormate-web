...
function resetTodayData() {
    if (confirm('⚠️ This will reset today\'s points and session data. Continue?')) {
        ...
        if (sessionTracking[today]) {
            sessionTracking[today] = { sessions: [], activeSession: null };
        }
...
// CONNECTION FIX: Function to enable role selection buttons even if Firebase fails
function enableRoleSelectionButtons() {
    console.log('🎯 Enabling role selection buttons...');

    // Force enable all clickable elements
    ...
    clickableSelectors.forEach(selector => {
        ...
        elements.forEach(element => {
            ...
            if (!element.onclick && (element.getAttribute('onclick') || element.getAttribute('data-role'))) {
                element.addEventListener('click', function(e) {
                    ...
                    if (roleAttr) {
                        selectRole(roleAttr);
                    } else if (onclickAttr && onclickAttr.includes('selectRole')) {
                        const role = onclickAttr.includes('student') ? 'student' : 'mentor';
                        selectRole(role);
                    }
                });
            }
    ...
    // Hide connecting messages
    const connectingElements = document.querySelectorAll('[class*="connecting"], [class*="loading"]');
    connectingElements.forEach(element => {
        if (element.textContent && element.textContent.toLowerCase().includes('connecting')) {
            element.style.display = 'none';
        }
    });

    // Make sure role selection screen is visible
    const roleScreen = document.getElementById('roleSelectionScreen');
    if (roleScreen) {
        roleScreen.style.display = 'block';
        roleScreen.style.pointerEvents = 'auto';
    }

    console.log('✅ Role selection buttons enabled successfully');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('📱 Device online status:', navigator.onLine);

    // Initialize Firebase with enhanced error handling
    initializeFirebase();

    // Start other core functions
    startTimeUpdates();
    checkAndResetDailyRoutines();

    // CRITICAL: Backup timer to force-enable buttons after 3 seconds
    setTimeout(() => {
        console.log('🔧 Backup timer: Force-enabling role selection buttons');
        enableRoleSelectionButtons();
    }, 3000);

    // Another backup timer for slow connections
    setTimeout(() => {
        if (!currentUser) {
            console.log('🚨 Emergency backup: Re-enabling buttons after 8 seconds');
            enableRoleSelectionButtons();
        }
    }, 8000);

    // Check for active sessions after everything loads
    setTimeout(() => {
        if (typeof checkSessionStateWithValidation === 'function') {
...
// FIXED: Session State Management with Validation
    ...
    if (sessionTracking[today] && sessionTracking[today].activeSession) {
        ...
        if (validateSavedSession(savedSession)) {
            ...
            if (savedSession.startTime) {
                ...
                // FIXED: Validate elapsed time to prevent explosion
...
function initializeFirebase() {
    try {
        ...
        // Connection timeout - enable buttons after 5 seconds regardless
        const connectionTimeout = setTimeout(() => {
            console.warn('⚠️ Firebase connection timeout - enabling offline mode');
            isFirebaseConnected = false;
            updateFirebaseStatus(false);
            enableRoleSelectionButtons(); // CRITICAL: Enable buttons
        ...
        database.ref('.info/connected').on('value', function(snapshot) {
            ...
            if (connected) {
                ...
                console.log('📱 Firebase disconnected - using offline mode');
                initializeDefaultData();
            }

            // ALWAYS enable role selection buttons
            enableRoleSelectionButtons();
        });
        ...
        // CRITICAL: Enable buttons even on complete failure
        enableRoleSelectionButtons();

        alert('Firebase connection failed. App will run in offline mode with limited functionality.');
    }
}
...
function updateSessionTimerSafely() {
    ...
    if (lastUpdateTime && (now - lastUpdateTime) > 5000) {
        // If more than 5 seconds since last update, something is wrong
...
// MENTOR FUNCTIONS (same as before but with validation)
...
function updateStudentStats() {
    const today = getTodayDateString();
    const todayPoints = validatePoints(dailyPoints[today] || 0); // FIXED: Validate points
    const todaySessions = sessionTracking[today];

    // Calculate today's stats with validation
    let completedToday = 0;
    let totalStudyTime = 0;

    if (todaySessions && todaySessions.sessions) {
        completedToday = todaySessions.sessions.length;
        totalStudyTime = todaySessions.sessions.reduce((sum, session) => {
            return sum + validateSessionTime(session.actualDuration || 0); // FIXED: Validate each session time
        }, 0);
    }

    // Add active session time with validation
    if (activeSession && sessionState === 'active' && sessionValidated) {
        totalStudyTime += sessionElapsedTime;
    }

    totalStudyTime = validateSessionTime(totalStudyTime); // FIXED: Final validation
...
function updateWeeklyStats() {
    ...
    // Calculate weekly totals with validation
    for (let i = 0; i < 7; i++) {
        const date = new Date();
        ...
        if (dailyPoints[dateStr]) {
            weeklyPoints += validatePoints(dailyPoints[dateStr]); // FIXED: Validate points
        }

        // Sessions and time with validation
        if (sessionTracking[dateStr] && sessionTracking[dateStr].sessions) {
            weeklySessions += sessionTracking[dateStr].sessions.length;
            const dayTime = sessionTracking[dateStr].sessions.reduce((sum, session) => {
                return sum + validateSessionTime(session.actualDuration || 0); // FIXED: Validate time
            }, 0);
            weeklyHours += dayTime;

            sessionTracking[dateStr].sessions.forEach(session => {
                if (session.efficiency) {
                    totalEfficiency += Math.min(session.efficiency, 200); // Cap efficiency
                    efficiencyCount++;
                }
            });
        }
    ...
    weeklyPoints = validatePoints(weeklyPoints); // FIXED: Final validation
    weeklyHours = validateSessionTime(weeklyHours); // FIXED: Final validation
...
// Continue with remaining functions (same as before but with validation where needed)...
...
function renderDailyChart() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const points = validatePoints(dailyPoints[dateStr] || 0); // FIXED: Validate points
        const studyTime = sessionTracking[dateStr] ? 
            validateSessionTime(sessionTracking[dateStr].sessions?.reduce((sum, s) => sum + (s.actualDuration || 0), 0) || 0) : 0; // FIXED: Validate time
...
// [Rest of the functions remain the same - test functions, mentor functions, etc.]
...
function updateMentorAnalytics() {
    ...
    // Today's performance with validation
    const todayPoints = validatePoints(dailyPoints[today] || 0);
    const todaySessions = sessionTracking[today];
    ...
    if (todaySessions && todaySessions.sessions) {
        ...
        todaySessionTime = todaySessions.sessions.reduce((sum, session) => {
            return sum + validateSessionTime(session.actualDuration || 0);
        }, 0);
    }
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (dailyPoints[dateStr]) {
            weeklyPoints += validatePoints(dailyPoints[dateStr]);
        }
        ...
        if (sessionTracking[dateStr] && sessionTracking[dateStr].sessions) {
            ...
            weeklyStudyTime += dayTime;
        }
    }

    weeklyPoints = validatePoints(weeklyPoints);
    weeklyStudyTime = validateSessionTime(weeklyStudyTime);
...
function getLastWeekPoints() {
    ...
    for (let i = 7; i < 14; i++) {
        ...
        const dateStr = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
        if (dailyPoints[dateStr]) {
            lastWeekPoints += validatePoints(dailyPoints[dateStr]);
        }
    }
...
function renderDailyPointsChart() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const dateStr = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
        const dayName = date.toLocaleDateString('en-US', { weekday: 'short' });
        const points = validatePoints(dailyPoints[dateStr] || 0);
...
function renderStudyTimeChart() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const dayName = date.toLocaleDateString('en-US', { weekday: 'short' });
        const studyTime = sessionTracking[dateStr] ? 
            validateSessionTime(sessionTracking[dateStr].sessions?.reduce((sum, s) => sum + (s.actualDuration || 0), 0) || 0) : 0;
...
function renderPerformanceTable() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const studyTime = validateSessionTime(sessions.reduce((sum, s) => sum + (s.actualDuration || 0), 0));
        const avgEfficiency = sessionsCount > 0 ? 
            Math.round(sessions.reduce((sum, s) => sum + Math.min(s.efficiency || 0, 200), 0) / sessionsCount) : 0;
...
function renderLiveSessionsMonitor() {
    ...
    let elapsed = 0;
    if (sessionState === 'active' && sessionValidated) {
        elapsed = sessionElapsedTime;
    } else {
        // For paused sessions, calculate from saved data
        const startTime = new Date(activeSession.startTime).getTime();
        const currentTime = Date.now();
        const rawElapsed = Math.floor((currentTime - startTime - (activeSession.totalPausedTime || 0)) / 60000);
        elapsed = validateSessionTime(rawElapsed);
    }
            ...
            <div class="live-session-body">
                ...
                <div class="session-stats">
                    <div class="stat">
                        ...
                        <span class="stat-value">${validatePoints(elapsed * POINTS_PER_MINUTE)}</span>
                    </div>
                    <div class="stat">
        ...
        <div class="routine-list">
            <div class="section-header">
                ...
                <div style="display: flex; gap: 10px;">
                    ...
                    <button onclick="resetTodayData()" class="btn btn-warning btn-small">🔄 Reset Today</button>
                    ${currentUser && currentUser.role === 'mentor' ? '<button onclick="resetAllData()" class="btn btn-danger btn-small">⚠️ Reset All</button>' : ''}
                </div>
            </div>
...
// Modal Functions, Test Functions, and other remaining functions
// (Same as previous version - these don't need validation changes)
...
function saveRoutine() {
    ...
    const duration = Math.min(parseInt(document.getElementById('routineDuration').value) || 60, MAX_SESSION_TIME);
    const points = Math.min(parseInt(document.getElementById('routinePoints').value) || 12, duration * POINTS_PER_MINUTE);
...
function updateRoutine() {
    ...
    const duration = Math.min(parseInt(document.getElementById('editRoutineDuration').value) || 60, MAX_SESSION_TIME);
    const points = Math.min(parseInt(document.getElementById('editRoutinePoints').value) || 12, duration * POINTS_PER_MINUTE);
...
console.log('🚀 MentorMate Pro - FINAL FIXED POINTS/TIME EXPLOSION BUG! 💯');
console.log('🔧 KEY SHORTCUTS:');
console.log('   - Ctrl+Shift+R (Mentor only): Reset all data');
