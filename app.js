...
function startTimeUpdates() {
    ...
    setInterval(updateSessionTimer, 1000); // Update session timer every second
}
...
// SCREEN MANAGEMENT - Same as before
...
// PROFESSIONAL SESSION TRACKING SYSTEM
...
function startSession(routineId) {
    ...
    saveToFirebase('sessionTracking', {
        ...sessionTracking,
        [getTodayDateString()]: {
            ...sessionTracking[getTodayDateString()],
    ...
    console.log('🎯 Session started:', routine.name);
}

function pauseSession() {
    if (!activeSession || !sessionTimer) return;
...
function updateActiveSessionDisplay() {
    ...
    if (statusElement) {
        statusElement.textContent = activeSession.status === 'active' ? 'Active' : 'Paused';
    }
}
...
function loadStudentData() {
    ...
    // Check for active session from previous browser session
    const today = getTodayDateString();
    if (sessionTracking[today] && sessionTracking[today].activeSession) {
        // Resume active session display but don't auto-resume timer
        // User will need to manually resume
        activeSession = sessionTracking[today].activeSession;
        showPausedSessionResume();
    }
}
        ...
        if (routine.status === 'completed') {
            ...
            switch(sessionStatus) {
                ...
                case 'live':
                    ...
                    buttonDisabled = activeSession !== null; // Disable if another session is active
                    buttonClass = activeSession ? 'btn-secondary' : 'btn-success';
                    break;
                case 'missed':
...
function renderLiveSessionsMonitor() {
    ...
    if (!todaySessions || !todaySessions.activeSession) {
        container.innerHTML = `
        ...
        <div class="live-session-card">
            <div class="live-session-header">
                ...
                <div class="session-status ${activeSession.status}">
                    ${activeSession.status === 'active' ? '🟢 Active' : '⏸️ Paused'}
                </div>
            </div>
            <div class="live-session-body">
                ...
                <div class="session-stats">
                    ...
                    <div class="stat">
                        ...
                        <span class="stat-value">${formatTime(Math.floor((activeSession.totalPausedTime || 0) / 60000))}</span>
                    </div>
                </div>
            </div>
        </div>
...
console.log('🚀 MentorMate Pro - Professional Session Tracking System Loaded!');