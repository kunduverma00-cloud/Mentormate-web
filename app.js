...
const firebaseConfig = {
  ...
  databaseURL: "https://mentor-web-6b4c8-default-rtdb.firebaseio.com",  // ✅ ADD THIS
...
// Firebase Initialization with PROPER DATA LOADING
function initializeFirebase() {
    try {
        ...
        database.ref('.info/connected').on('value', function(snapshot) {
            ...
            if (isFirebaseConnected && !dataLoaded) {
...
// FIXED: Load all data from Firebase with proper structure
function loadAllDataFromFirebase() {
    console.log('Loading all data from Firebase...');

    const promises = [
        loadFirebaseData('routines'),
        loadFirebaseData('tests'), 
        loadFirebaseData('testResults'),
        loadFirebaseData('notifications'),
        loadFirebaseData('punishmentRules'),
        loadFirebaseData('studentPunishments')
    ];
    ...
    Promise.all(promises).then(() => {
        ...
        // Refresh UI if user is already selected
        if (currentUser) {
...
function loadFirebaseData(dataType) {
        ...
        database.ref(dataType).once('value').then(snapshot => {
            if (snapshot.exists()) {
                ...
                if (Array.isArray(data)) {
                    ...
                    window[dataType] = Object.values(data);
                } else {
                    window[dataType] = [];
                }
                console.log(`Loaded ${dataType}:`, window[dataType]);
            } else {
                window[dataType] = [];
                console.log(`No ${dataType} found in Firebase`);
            }
            resolve();
        }).catch(error => {
            console.error(`Error loading ${dataType}:`, error);
            window[dataType] = [];
            resolve(); // Don't reject, just continue with empty array
        });
    });
}

// IMPROVED: Save data to Firebase with better error handling
function saveToFirebase(path, data) {
    if (database && isFirebaseConnected) {
        // Ensure data is an array or object
        const dataToSave = Array.isArray(data) ? data : (data || []);

        database.ref(path).set(dataToSave).then(() => {
            console.log(`✅ Data saved to Firebase: ${path}`);
        }).catch(error => {
            console.error(`❌ Error saving to Firebase ${path}:`, error);
            // Try to save again after a delay
            setTimeout(() => {
                database.ref(path).set(dataToSave).catch(err => {
                    console.error(`❌ Retry failed for ${path}:`, err);
                });
            }, 2000);
        });
    } else {
        console.log('⚠️ Firebase not connected, data saved locally only');
    }
}
...
// AUTOMATIC CHECK-IN WITH REAL TIME CALCULATION
